...
class BicycleModel(torch.nn.Module):
    ...
    def forward(self, state: Dict[str, torch.TensorType], actions: Dict[str, torch.TensorType], track_token:str, iter:int, plotter=True): # discarding the is_terminated option for now
        ...
        ideal_steering_angle = state["steering_angle"][:,:,0] + self.delta_t * actions['steer'][:,:,0] # (1,1)
        ...
        ideal_accel = actions['throttle'][:,:,0] # (1,1)
        # ideal_accel = accel
        ...
        longitudinal_speed = state["speed"][:,:,0] # (1,1)
        ...
        longitudinal_speed = longitudinal_speed + updated_accel * self.delta_t # (1,1)
        lhs = (state['yaw'][:,:,0] + longitudinal_speed/wheel_base*self.delta_t*torch.sin(beta) - min_) % (2 * torch.pi) + min_ # (1,1)
        state['vel'] = torch.cat([torch.unsqueeze(longitudinal_speed*torch.cos(lhs), dim=-1), torch.unsqueeze(longitudinal_speed*torch.sin(lhs), dim=-1)], dim=-1) # (1,1,1)
        state['yaw'] = torch.unsqueeze(lhs, dim=-1) # (1,1,1)
        ...
        state['speed'] = torch.unsqueeze(longitudinal_speed, dim=-1)
    ...
    def forward_all(self, state: Dict[str, torch.TensorType], actions: Dict[str, torch.TensorType]): # discarding the is_terminated option for now
        ...
        # beta = updated_steering_rate
        beta = torch.atan(torch.tan(updated_steering_rate)/2)
        print('helloooooo ', updated_steering_rate.item(), '   ', beta.item(), '   ', state['yaw'].item())
        ...
        lhs = (state['yaw'][:,:,0] + longitudinal_speed/wheel_base*self.delta_t*torch.sin(beta) - min_) % (2 * torch.pi) + min_ # as in king's version of bm
        ...
        x_dot = torch.cos(state["yaw"][:,:,0])*longitudinal_speed
        y_dot = torch.sin(state["yaw"][:,:,0])*longitudinal_speed
        print('ideal accel ', ideal_accel.item())
        print('haha ', (state["yaw"][:,:,0]+beta).item(), '   ', beta.item())
        print('this is the x dot ', x_dot.item(), '    ', longitudinal_speed.item())
        print('this is the y dot ', y_dot.item())
        state['pos'] = torch.cat([torch.unsqueeze(state['pos'][:,:,0] + x_dot*self.delta_t, dim=-1), torch.unsqueeze(state['pos'][:,:,1] + y_dot*self.delta_t, dim=-1)], dim=-1)
