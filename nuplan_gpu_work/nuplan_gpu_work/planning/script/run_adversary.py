...
from nuplan_gpu_work.planning.simulation.adversary_optimizer.executor import run_optimization

from nuplan.common.utils.s3_utils import is_s3_path
from nuplan_gpu_work.planning.script.builders.adversary_builder import build_adversary
...
from nuplan.planning.script.utils import set_default_path, set_up_common_builder
...
def run_adversary(cfg: DictConfig, planners: Optional[Union[AbstractPlanner, List[AbstractPlanner]]] = None) -> None:
    """
    ...
    profiler_name = 'building_adversary'
    ...
    my_simulation, my_planner = build_adversary(
        cfg=cfg,
        callbacks=callbacks,
    ...
    assert isinstance(my_simulation, Simulation), f'the simulation is not of the right type: {type(my_simulation)}'
    assert isinstance(my_planner, AbstractPlanner), f'the planner is not of the right type: {type(my_planner)}'
    ...
    logger.info('Running adversary optimization...')
    # where to indicate that we want to use the tracker that we want for the optimizer? 
    report = run_optimization(my_simulation, my_planner, cfg=cfg, profiler_name='running_adversary_optimization')
    # save_runner_reports(report, common_builder.output_dir, cfg.runner_report_file)
    logger.info('Finished running adversary optimization!')
