    ...
    if future_trajectory_sampling and len(tracked_objects) > 0:
        ...
        for key in agent_future_trajectories:
            ...
            if len(agent_future_trajectories[key]) == 1:
                ...
                interpolated_future_waypoints = interpolate_future_waypoints(
...
def extract_tracked_objects_original_for_modified(
    token: str,
    log_file: str,
    future_trajectory_sampling: Optional[TrajectorySampling] = None,
) -> TrackedObjects:
    ...
    if future_trajectory_sampling and len(tracked_objects) > 0:
        ...
        for key in agent_future_trajectories:
            ...
            if len(agent_future_trajectories[key]) == 1:
                ...
                '''
                print(f'we have the time horizon! {future_trajectory_sampling.time_horizon}')
                print(f'the len before interpolation is {len(agent_future_trajectories[key])}')
                ...
                print(f'the len after interpolation is {len(interpolated_future_waypoints)}')
                ...
                # print(f'and the length after the interpolation {len(tracked_objects[agent_indexes[key]]._predictions[0].valid_waypoints)}')
                '''
                tracked_objects[agent_indexes[key]]._predictions = [
                    PredictedTrajectory(
                        1.0,
...
# adding function to modify the trajectories read from the database, given a new parameter
def modified_extract_tracked_objects(
    token: str,
    log_file: str,
    ...
    Extracts all boxes from a lidarpc.
    :param lidar_pc: Input lidarpc.
    :param changed_current_center: a dict of agent tokens along with their updated current center.
    :param future_trajectory_sampling: If provided, the future trajectory sampling to use for future waypoints.
    :return: Tracked objects contained in the lidarpc.
    """
    tracked_objects: List[TrackedObject] = []
    ...
    for idx, tracked_object in enumerate(get_tracked_objects_for_lidarpc_token_from_db(log_file, token)):
        ...
        if tracked_object.track_token in changed_current_center.keys():
            tracked_object = Agent.from_new_pose(tracked_object, changed_current_center[tracked_object.metadata.track_token])
        tracked_objects.append(tracked_object)
    ...
    if future_trajectory_sampling and len(tracked_objects) > 0:
        ...
        for track_token, waypoint in get_future_waypoints_for_agents_from_db(
            ...
            if track_token in changed_current_center.keys() and agents_waypoint_counter[track_token]==0:
                counter = agents_waypoint_counter[track_token]
                new_box = OrientedBox.from_new_pose(waypoint.oriented_box, changed_current_center[track_token])
...
# adding function to modify the trajectories read from the database, given a new parameter
def modified_extract_tracked_objects_random_perturbation(
    token: str,
    log_file: str,
    ...
    Extracts all boxes from a lidarpc.
    :param lidar_pc: Input lidarpc.
    :param changed_predictions: a dict of agent tokens along with their updated predictions
    :param future_trajectory_sampling: If provided, the future trajectory sampling to use for future waypoints.
    :return: Tracked objects contained in the lidarpc.
    """
    tracked_objects: List[TrackedObject] = []
    ...
    for idx, tracked_object in enumerate(get_tracked_objects_for_lidarpc_token_from_db(log_file, token)):
        if future_trajectory_sampling and isinstance(tracked_object, Agent):
            ...
            agent_future_trajectories[tracked_object.metadata.track_token] = []
        # print(f'the changed_prediction {changed_prediction}')
        if tracked_object.track_token in changed_prediction.keys():
            temp_center = changed_prediction[tracked_object.track_token][0].center
            tracked_object = Agent.from_new_pose(tracked_object, StateSE2(temp_center.x, temp_center.y, temp_center.heading))
        tracked_objects.append(tracked_object)
        agents_waypoint_counter[tracked_object.metadata.track_token] = 0
    ...
    if future_trajectory_sampling and len(tracked_objects) > 0:
        ...
        for track_token, waypoint in get_future_waypoints_for_agents_from_db(
            ...
            if track_token in changed_prediction.keys():
            # agent_future_trajectories[track_token].append(waypoint)
                counter = agents_waypoint_counter[track_token]
                # it appears that there is a track_token that is not registered in the changed_prediction
                # print(changed_prediction[track_token])
                agent_future_trajectories[track_token].append(changed_prediction[track_token][counter])
                agents_waypoint_counter[track_token] = counter+1
            else:
                agent_future_trajectories[track_token].append(waypoint)
